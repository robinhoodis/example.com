  ...
  for SERVICENAME in ${!SERVICES[@]}; do
    ...
    for SITENAME in ${!SITES[@]}; do
      for CONTAINER in 1 2 3; do
        ...
        echo "docker run -d --name ${SITENAME}.${SERVICENAME}${CONTAINER} --hostname ${SITENAME}.${SERVICENAME}${CONTAINER}.${DOMAIN} --net ${SITENAME} --ip ${SITES[$SITENAME]}.${i}${CONTAINER} --rm ${SERVICES[$SERVICENAME]} examplecom/service"
        docker run -d --name ${SITENAME}.${SERVICENAME}${CONTAINER}  --hostname ${SITENAME}.${SERVICENAME}${CONTAINER}.${DOMAIN} --net ${SITENAME} --ip ${SITES[$SITENAME]}.${i}${CONTAINER} --rm ${SERVICES[$SERVICENAME]} examplecom/service
      done
    done
  ((i++))
  done
