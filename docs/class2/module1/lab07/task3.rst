...
You have seen how easy to is to create new configurations on the BIG-IP DNS via iControlREST using Postman. So far, we have used to Postman to configure object types that we have already configured in the Web interface. In this step we will create a brand new configuration element that is relevant to both of our Wide-IPs; a Link.
